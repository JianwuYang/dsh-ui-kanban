...
export const zh = {
  ...
  cardNotExposed: '配置命名空间 dsh-kanban 当前不可用', cardNotExposedBody: '浏览器侧拿不到该命名空间的设置快照：宿主未注册该命名空间，或当前连接把偏好保存在进程内（memory 模式，只读）。host 半边不受影响：kanban-* 工具仍实时读取配置。',
...
export const en: Record<TKey, string> = {
  ...
  cardNotExposed: 'Settings namespace dsh-kanban is unavailable', cardNotExposedBody: 'The browser cannot read a snapshot of this namespace: the host registers no such namespace, or this connection keeps preferences process-local (memory mode, read-only). The host half is unaffected: kanban-* tools keep reading the config live.',
