...
客户端半边（`src/client/`）通过 `ctx.slots` 注册五个 UI 面，全部是纯声明式插槽注册，
**无需**改动 harness 源码即可在任何 harness 上工作；配置卡片的数据路径经
`ctx.settingsScope` 读取命名空间——harness 自 2026-08-12 起「注册即暴露」，
旧的 `WEB_SETTINGS_NAMESPACES` 白名单已删除。

| 面 | 模块 | Id / Key | 行为 |
|----|------|----------|------|
| `settings.plugin.item` | `src/client/config-card.ts` | key = `dsh-kanban` | 设置 → 插件 → Configurable 下的一张卡片。编辑**全局 Jira/GitLab host + token**（token 是密码框；留空保留 host 侧密钥，不会覆盖已脱敏的 token）以及标量设置（`dataDir`、`allowSelfSigned`、`verbose`）。全局 host/token 经 settings 命名空间自身写入（`settingsScope.mutate` 路径操作，每次只写一个叶子）；工作区级的项目/连接覆盖用 `kanban-configure` 工具管理。 |
...
看板/列表端点接受可选的 `?workspace=<id|title|path>` 查询（定位某个工作区）、
`?session=<id>`（调用方会话，工作区归属的权威来源）或 `?cwd=<path>`（会话的工作区）
——会话级悬浮面板在 `GET /issues` 和 `POST /sync` 上用其中之一来显示按会话区分的
项目。缺省时回退到第一个工作区。在工作区模型下，看板从每个 DSH 工作区派生一个项目，
全局 Jira/GitLab host+token 位于命名空间顶层。
...
| `modal.tsx` | 应用内 `Modal`（模块级深度栈保证 Esc 只关最顶层的嵌套弹窗、焦点陷阱 + 恢复、body 滚动锁、进出场动画；`footer={null}` 可去掉底栏）、`ConfirmDialog`/`ChoiceDialog` + `DialogsProvider`——替代所有原生 `alert`/`confirm`/`prompt`。 |
...
| `session-send.ts` | 经官方 `ISession.prompt`（当前会话）与 `uiWorkspace.startSession()`（当前工作区新建会话）实现「丢进会话分析」；图片附件以 image content part（base64）随消息发出。 |
...
（`session-send.ts`；新建会话走 `uiWorkspace.startSession()`）。消息只带 issue key +
...
配置卡片在任何 harness 构建上都会渲染。注册 settings 命名空间本身就是暴露：
自 2026-08-12 起 web 网关服务 `ctx.settings.describe()` 返回的每一个命名空间，
旧的 `WEB_SETTINGS_NAMESPACES` 白名单与 `settings-not-exposed` 错误码都已删除。
卡片只在浏览器完全拿不到命名空间快照时显示「不可用」状态卡——宿主没有注册该
命名空间，或当前连接把偏好保存在进程内（memory 模式，只读）。无论哪种情况，
host 半边在每次工具调用时仍然读取解析后的命名空间，工具照常工作。
